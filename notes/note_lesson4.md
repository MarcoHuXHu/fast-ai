...
#### 基本思路
Movielens的dataset中选择n个用户, 对m部电影的评分(没有评分记作0). 这样得到了一个n\*m的矩阵. 对于每一个用户, 随机初始化一个长度为a的向量, 代表用户的属性; 对于每一步电影, 同样的随机初始化一个长度为a的向量, 代表电影的属性. 这样全部用户属性由一个n\*a的矩阵表示; 全部电影的属性由一个a\*m的矩阵表示. 对这两个矩阵坐点乘, 得到了一个与用户对电影评价相同的n\*m的矩阵.  
